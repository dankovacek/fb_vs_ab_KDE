...
The Silverman estimator uses [KDEpy](https://kdepy.readthedocs.io/en/latest/) [@kdepy2023], which evaluates the kernel via FFT convolution in $O(n \log n)$.
...
The analysis covers six regional subsets drawn from the [Caravan](https://caravan-hydrology.github.io/) large-sample hydrology dataset [@kratzert2023caravan; @kratzert_2025_caravancsv]:

| Region | Subset | Reference |
|--------|--------|-----------|
| Australia | CAMELS-AUS | @fowler2021camels |
| Brazil | CAMELS-BR | @chagas2020camels |
| Chile | CAMELS-CL | @alvarez2018camels |
| Great Britain | CAMELS-GB | @coxon2020camels |
| North America | HYSETS | @arsenault2020comprehensive |
| Central Europe | LaMAH-CE | @klingler2021lamah |
...
[README.md](README.md) is generated from [README.src.md](README.src.md) using [Pandoc](https://pandoc.org/) with `citeproc`. To update the rendered README after editing the source:

```bash
make readme
```

Citation keys in `README.src.md` follow Pandoc's `[@key]` syntax. The bibliography is maintained in [references.bib](references.bib). To add a reference, add the BibTeX entry to `references.bib` and insert the citation key in `README.src.md`, then re-run `make readme`.
